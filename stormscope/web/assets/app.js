...
/* Repli réseau : si le contrat live ne fournit pas de stations exploitables mais
   que la maille de grille est présente, on reconstitue le réseau depuis les
   stations configurées (positions réelles) et on dérive la série horaire de la
   maille la plus proche — même surface modèle déjà affichée, pas de fabrication. */
function nearestGridCell(lat,lon,gridArr){let best=null,bd=Infinity;const cos=Math.cos(lat*Math.PI/180);for(const g of gridArr){const dy=g[0]-lat,dx=(g[1]-lon)*cos,d=dy*dy+dx*dx;if(d<bd){bd=d;best=g;}}return best;}
function stationsFromConfigGrid(R,gridArr){if(!gridArr||!gridArr.length)return [];return ((R&&R.stations)||[]).map(s=>{const lat=Number(s[1]),lon=Number(s[2]);if(!Number.isFinite(lat)||!Number.isFinite(lon))return null;const cell=nearestGridCell(lat,lon,gridArr);if(!cell)return null;return {name:s[0],lat,lon,source:'maillage modèle',h:cell[2]};}).filter(Boolean);}
...
function dashboardFromMapLivePayload(payload,R){
  ...
  if(!out.stations.length&&out.grid.length)out.stations=stationsFromConfigGrid(R,out.grid);
  return out;
}
...
function applyMapLivePayload(S,payload,R){const hours=(payload.hours||[]).slice(0,48);S.off=0;S.hours=hours.map(h=>h.time||h.hour||'');S.hour_scores=hours.map(h=>numberOrNull(h.score??h.max_score??h.mean_score));S.grid=(payload.grid||[]).filter(p=>p.lat!=null&&p.lon!=null).map(p=>[Number(p.lat),Number(p.lon),apiHourlyFromMapItem(p,hours)]);S.stations=(payload.stations||[]).filter(s=>s.lat!=null&&s.lon!=null).map(s=>({name:s.name||s.station_id||'station',lat:Number(s.lat),lon:Number(s.lon),source:'alert-watch',score:s.score,signals:s.signals||[],h:apiHourlyFromMapItem(s,hours)}));if(!S.stations.length&&S.grid.length)S.stations=stationsFromConfigGrid(R,S.grid);S.source=payload.contract==='meteovoid_country_live_v1'?'country-contract-live':'alert-watch-live';S.timeline=payload.timeline||{};S.lastMapRefresh=new Date();const sl=document.getElementById(S.ids.time);if(sl){sl.max=Math.max(S.hours.length-1,0);sl.value=0;}S.i=0;ensureStaticMapFallback(S,S.ids.eye?'euMap':'carteMap',R);mapRender(S);if(S===CARTE){installDashboardFromMapLive(payload,R);}return S.grid.length>0||S.stations.length>0;}
