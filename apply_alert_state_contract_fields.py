...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count == 0:
        ...
        if label == "alert_state fields" and '"notification_allowed": notification_allowed' in text:
            return text
        if (
            label == "notification_state fields"
            and '"notification_allowed": bool(operational.get("notification_allowed", should_notify))'
            in text
        ):
            return text
    ...
    if count > 1:
        raise SystemExit(
            f"Patch anchor for {label} appears {count} times; refusing ambiguous edit."
        )
...
def main() -> int:
    ...
    text = replace_once(
        text, NOTIFICATION_ANCHOR, NOTIFICATION_REPLACEMENT, "notification_state fields"
    )
