...
function renderRadar(){return `<div class="section-title">Carte Europe live</div><div class="grid cards2"><div class="card"><div id="europeMap" class="live-map" aria-label="Carte RainViewer Europe"></div><div class="map-actions"><button id="euRecenter" type="button">Recentrer Europe</button><button id="euToggleRain" type="button" aria-pressed="true">Radar RainViewer</button></div><div class="map-status" id="euMapStatus">initialisation carte Europe…</div></div><div class="card"><div class="eyebrow">Carte Europe</div><h2>Radar direct + pays amont</h2><p class="muted">La couche RainViewer est chargée côté navigateur. Les rectangles pays viennent du modèle Europe et les popups affichent sources, statut et priorité Belgique.</p><div class="links"><a href="reports/latest/european_national_radar_map.html">Carte nationale Europe</a><a href="reports/latest/rainviewer_radar_map.html">RainViewer</a><a href="reports/latest/european_upstream_map.html">Europe amont</a><a href="reports/latest/european_national_radar_report.md">Rapport</a></div></div></div><div class="section-title">Couches radar et fallback</div><div class="grid cards4">${(M.radar_layers||[]).map(layerCard).join('')}</div>`}
