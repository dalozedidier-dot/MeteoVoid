...
def _write_europe_convective_map(site_dir: Path) -> None:
    """Publish the standalone Europe convective map used by the Europe page.

    The file is intentionally kept outside the Python template because it is a
    self-contained Leaflet application. It reads live Open-Meteo/RainViewer
    data in the browser and does not depend on the MeteoVoid static JSON API.
    """
    source = (
        Path(__file__).resolve().parents[1] / "static" / "europe_convective_map.html"
    )
    if not source.exists():
        return
    html = source.read_text(encoding="utf-8")
    for name in ("carte-europe.html", "europe_convective_map.html"):
...
def build_index(report_dir: Path, site_dir: Path) -> dict[str, Any]:
    ...
    _write_europe_convective_map(site_dir)
...
function renderRadar(){return `<div class="section-title">Carte de bascule convective · Europe</div><div class="card iframe-card"><iframe src="carte-europe.html" title="Carte de bascule convective Europe · Open-Meteo · RainViewer" loading="lazy"></iframe></div><div class="grid cards2" style="margin-top:14px"><div class="card"><div class="eyebrow">Carte intégrée</div><h2>Surface convective + radar live</h2><p class="muted">Cette carte embarque la vue Europe complète : zones, Belgique, pays voisins, surface de bascule, détecteurs, RainViewer, satellite IR, réseau radar national et foudre communautaire quand l'intégration est autorisée.</p><div class="links"><a href="carte-europe.html">Ouvrir plein écran</a><a href="reports/latest/european_national_radar_map.html">Carte nationale Europe</a><a href="reports/latest/rainviewer_radar_map.html">RainViewer</a><a href="reports/latest/european_upstream_map.html">Europe amont</a><a href="reports/latest/european_national_radar_report.md">Rapport</a></div></div><div class="card"><div class="eyebrow">Lecture honnête</div><h2>Affichage ≠ preuve machine</h2><p class="muted">OpenStreetMap/CARTO reste le fond de carte. RainViewer et satellite sont des couches d'affichage. La surface de bascule est un proxy d'ingrédients calculé en direct depuis Open-Meteo. Une preuve radar machine exige une trame radar lisible et des métriques calculées.</p></div></div><div class="section-title">Couches radar et fallback</div><div class="grid cards4">${(M.radar_layers||[]).map(layerCard).join('')}</div>`}
...
let EU_MAP=null;let EU_RAIN_LAYER=null;let EU_RAIN_VISIBLE=true;let EU_RAIN_REFRESH=null;
function euStatus(txt){const e=document.getElementById('euMapStatus');if(e)e.textContent=txt;}
function euTileUrl(data,frame){const host=String((data&&data.host)||'https://tilecache.rainviewer.com').replace(/\/$/,'');const path=String((frame&&frame.path)||'');if(!path)throw new Error('rainviewer_no_frame_path');return `${host}${path.startsWith('/')?path:'/'+path}/256/{z}/{x}/{y}/2/1_1.png`;}
function addEuropeRainViewer(){if(!EU_MAP||typeof L==='undefined')return;euStatus('RainViewer direct · chargement');fetch('https://api.rainviewer.com/public/weather-maps.json',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('rainviewer_http'))).then(data=>{const past=(data&&data.radar&&data.radar.past)||[];const nowcast=(data&&data.radar&&data.radar.nowcast)||[];const frames=past.concat(nowcast);const frame=frames[frames.length-1]||past[past.length-1];if(!frame||!frame.path)throw new Error('rainviewer_no_frame');const next=L.tileLayer(euTileUrl(data,frame),{opacity:.68,zIndex:230,attribution:'RainViewer',crossOrigin:true});if(EU_RAIN_VISIBLE)next.addTo(EU_MAP);const old=EU_RAIN_LAYER;EU_RAIN_LAYER=next;if(old)setTimeout(()=>{try{EU_MAP.removeLayer(old);}catch(e){}},250);const ts=frame.time?new Date(frame.time*1000):null;const stamp=ts?ts.toLocaleTimeString('fr-BE',{hour:'2-digit',minute:'2-digit'}):'actif';euStatus(`RainViewer direct · ${stamp}`);}).catch(()=>euStatus('RainViewer indisponible · fond Europe actif'));}
function initEuropeMap(){const target=document.getElementById('europeMap');if(!target)return;if(typeof L==='undefined'){euStatus('Leaflet indisponible · vérifier le CDN');return;}if(!EU_MAP){EU_MAP=L.map(target,{zoomControl:true,scrollWheelZoom:true,preferCanvas:true}).setView([48.6,6.0],5);L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap &copy; CARTO'}).addTo(EU_MAP);(M.countries||[]).forEach(c=>{const b=c.bbox||{};if(b.south==null||b.west==null||b.north==null||b.east==null)return;const col=c.machine_radar_available?'#1cb98a':'#e3b13b';const rect=L.rectangle([[b.south,b.west],[b.north,b.east]],{color:col,weight:1.6,opacity:.82,fillColor:col,fillOpacity:.08}).addTo(EU_MAP);rect.bindPopup(`<b>${esc(c.label)}</b><br>Priorité Belgique : ${esc(c.priority_for_belgium||'—')}<br>Sources : ${esc(c.configured_source_count||0)}/${esc(c.source_count||0)} prêtes<br>Preuve machine : ${c.machine_radar_available?'oui':'non'}<br><a href="${esc(c.country||'')}.html">ouvrir la page pays</a>`);});}setTimeout(()=>EU_MAP.invalidateSize(),120);addEuropeRainViewer();if(!EU_RAIN_REFRESH)EU_RAIN_REFRESH=setInterval(addEuropeRainViewer,300000);const rec=document.getElementById('euRecenter');if(rec)rec.onclick=()=>EU_MAP.setView([48.6,6.0],5);const tog=document.getElementById('euToggleRain');if(tog)tog.onclick=()=>{EU_RAIN_VISIBLE=!EU_RAIN_VISIBLE;tog.setAttribute('aria-pressed',String(EU_RAIN_VISIBLE));if(EU_RAIN_LAYER){EU_RAIN_VISIBLE?EU_RAIN_LAYER.addTo(EU_MAP):EU_MAP.removeLayer(EU_RAIN_LAYER);}euStatus(EU_RAIN_VISIBLE?'RainViewer direct · actif':'RainViewer masqué');};}
