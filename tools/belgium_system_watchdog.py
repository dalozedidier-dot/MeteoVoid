...
def _source_health(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if error_ratio == 0:
        ...
        state = "fragile"
    return {"station_count": len(stations), "source_ok_count": ok, "source_error_count": errors, "source_error_ratio": round(error_ratio, 6), "state": state}


def _observation_gap_status(report: dict[str, Any]) -> dict[str, Any]:
    integrations = report.get("integrations") if isinstance(report.get("integrations"), dict) else {}
    external = report.get("external_confirmation") if isinstance(report.get("external_confirmation"), dict) else {}
    return {
        "generated_at": report.get("generated_at"),
        "satellite": {"source": "EUMETSAT/MTG/RDT", "status": "connector_prepared_not_configured", "configured": False},
        "gnss_water_vapour": {"source": "E-GVAP/GNSS", "status": "connector_prepared_not_configured", "configured": False},
        "pressure_crowd": {"source": "Netatmo or compatible pressure network", "status": "connector_prepared_not_configured", "configured": False},
        "lightning": {"source": "community or licensed lightning feed", "status": "configured" if integrations.get("lightning_integrated") or external.get("lightning_confirmation") not in {None, "none"} else "connector_prepared_not_configured", "configured": bool(integrations.get("lightning_integrated"))},
        "radar": {"source": "RainViewer/nowcast or configured radar feed", "status": "configured_or_visual_layer" if integrations.get("radar_integrated") else "visual_layer_only", "configured": bool(integrations.get("radar_integrated"))},
...
def _render_html(data: dict[str, Any]) -> str:
    ...
    missing = data.get("missing_required_outputs", []) if isinstance(data.get("missing_required_outputs"), list) else []
    missing_html = "".join(f"<li>{html.escape(str(item))}</li>" for item in missing) or "<li>Aucune sortie obligatoire manquante.</li>"
...
def write_watchdog_outputs(report: dict[str, Any], out_dir: Path) -> dict[str, Any]:
    ...
    (out_dir / "self_watchdog.json").write_text(json.dumps(data, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
    (out_dir / "observation_gap_status.json").write_text(json.dumps(observations, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
